"""MongoDB query planning for the neutral :mod:`httk.data.query` protocols.
...
from httk.data.query import SearchResult, UnsupportedQueryError

if TYPE_CHECKING:
    from httk.data.mongo.store import MongoStore
...
class MongoSearcher:
        ...
        while pending:
            ...
            for variable in tuple(pending):
                ...
                if isinstance(source, MongoReference):
                    ...
                    continue
                from httk.data.mongo.mapping import collection_name_for
    ...
    def results(self, **outputs: Any) -> Any:
        """Return a materialized :class:`~httk.data.mongo.results.MongoResultSet` for this query."""
        selected = [_MongoOutput(name, value) for name, value in outputs.items()] if outputs else None
        return __import__("httk.data.mongo.results", fromlist=["MongoResultSet"]).MongoResultSet(self, selected)
