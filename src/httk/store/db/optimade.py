...
:func:`optimade_filter_searcher` builds a :class:`~httk.store.db.searcher.SqlSearcher`
...
:class:`~httk.store.db.schema.TableSchema` — the same derivation
:class:`~httk.store.db.entry_provider.StoreEntryProvider` uses to serve the
class (via the shared :func:`~httk.store.db.entry_provider.served_specs` /
:func:`~httk.store.db.entry_provider.auto_definition` helpers), so a filter
...
from httk.store.db.searcher import SqlColumn, SqlExpression, SqlSearcher, SqlVariable
...
def optimade_filter_searcher(
    ...
    *,
    ...
    :func:`~httk.store.db.entry_provider.served_specs`) is filterable as
    ...
    - ``<rtype>.id HAS "<rtype>-<sid>"`` filters over the field's foreign-key
      ...
      :class:`~httk.store.db.entry_provider.StoreEntryProvider` mints them;
