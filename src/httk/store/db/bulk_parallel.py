"""Parallel encode + shard-merge backend for :class:`~httk.store.db.bulk.BulkIngest`.

This module implements the ``workers > 1`` mode of
:meth:`~httk.store.db.store.SqlStore.bulk_ingest`. The serial ``workers = 1``
path in :mod:`httk.store.db.bulk` is untouched; :class:`~httk.store.db.bulk.BulkIngest`
...
- **Workers** (``_worker_main``): forked processes that run the *pure* encoders
  ...
  :mod:`httk.store.db.store`) with a per-worker
...
class _WorkerEncoder:
    ...
    :meth:`~httk.store.db.bulk.BulkIngest._encode_active`: no table DDL, sids from
...
def merge(ingest: "httk.store.db.bulk.BulkIngest", manifests: list[_WorkerManifest]) -> None:
