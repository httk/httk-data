...
The rest of :mod:`httk.store.backend.sql` deals in generic SQLAlchemy objects.  This
...
def _fraction_inline(arguments: list[str]) -> str:
    ...
    :func:`httk.store.backend.sql.engine._fraction_scaled_equal`.  A ``NULL`` argument
