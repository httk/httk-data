...
class MongoSearcher:
    ...
    def results(self, **outputs: Any) -> Any:
        """Return a materialized :class:`~httk.store.mongo.results.MongoResultSet` for this query."""
        selected = [_MongoOutput(name, value) for name, value in outputs.items()] if outputs else None
        return __import__("httk.store.mongo.results", fromlist=["MongoResultSet"]).MongoResultSet(self, selected)
