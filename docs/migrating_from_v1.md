...
| httk v1 | *httk₂* |
| --- | --- |
| `httk.db.backend.Sqlite(path)` then `httk.db.store.SqlStore(backend)` | `Database.sqlite(path)` then `SqlStore(database)` |
...
store = SqlStore(Database.sqlite("results.sqlite"))
